...
class ScriptPreprocessor {
  ...
  String preprocess(String script) {
    ...
    for (final declaration in unit.declarations) {
      if (declaration is! FunctionDeclaration || declaration.externalKeyword == null) {
        continue;
      }

      final processor = _processors.cast<ExternalDeclarationProcessor?>().firstWhere(
    ...
    if (needsDartIo && !_hasDartIoImport(unit)) {
      ...
      edits.add(_SourceEdit(offset: insertionOffset, length: 0, replacement: _dartIoImportAt(unit, insertionOffset)));
    }

    return _applyEdits(script, edits);
  }

  static bool _hasDartIoImport(CompilationUnit unit) =>
      unit.directives.whereType<ImportDirective>().any((directive) => directive.uri.stringValue == 'dart:io');
  ...
  static String _dartIoImportAt(CompilationUnit unit, int offset) {
    ...
    final isBeforePart = unit.directives.whereType<PartDirective>().any((directive) => directive.offset == offset);
...
class _SourceEdit {
  const _SourceEdit({required this.offset, required this.length, required this.replacement});
